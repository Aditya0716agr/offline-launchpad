-- Seed data for new Supabase instance (Fixed version)
-- Run this AFTER creating a founder user in Supabase Auth

-- First, let's check if we have any founder profiles
-- If not, we'll create a temporary one for the sample data

-- Check if we have any founder profiles
DO $$
DECLARE
    founder_count INTEGER;
    temp_founder_id UUID;
BEGIN
    -- Count existing founder profiles
    SELECT COUNT(*) INTO founder_count FROM profiles WHERE role = 'founder';
    
    -- If no founder exists, create a temporary one
    IF founder_count = 0 THEN
        -- Create a temporary founder profile (you'll need to replace this with a real user_id)
        ...
        VALUES (
            gen_random_uuid(), -- This is a placeholder - you'll need to update this
            'Sample Founder',
            'founder',
            NOW(),
            NOW()
        ) RETURNING id INTO temp_founder_id;
        
        RAISE NOTICE 'Created temporary founder profile with ID: %', temp_founder_id;
        RAISE NOTICE 'IMPORTANT: You need to update this founder_id with a real user_id from your auth.users table!';
    ELSE
        RAISE NOTICE 'Founder profile already exists. Using existing founder.';
    END IF;
END $$;
