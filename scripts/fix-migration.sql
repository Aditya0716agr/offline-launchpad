-- Fixed migration script based on actual schema
...
-- Step 2: Update any existing profiles to have 'user' role
...
-- Step 4: Update the role column to use the enum
ALTER TABLE public.profiles ALTER COLUMN role TYPE user_role USING role::user_role;

-- Step 5: Update startup status to auto-approve all existing startups
UPDATE startups SET status = 'approved' WHERE status = 'pending';

-- Step 6: Drop existing restrictive policies (if they exist)
...
-- Step 7: Create new open policies for startups
...
-- Step 8: Update the auth trigger function
...
-- Step 9: Update the startup table to auto-approve new startups
ALTER TABLE public.startups ALTER COLUMN status SET DEFAULT 'approved';

-- Step 10: Add any missing categories if they don't exist
